...
def create_comprehensive_ranking_table(
    ...
    for model in sorted(individual_results.keys()):
        ...
        # Size column: example uses "80B(20B)" style if total!=active
        total_params = model_meta.get("total_params", "")
        active_params = model_meta.get("active_params", "")
        if total_params != "" and active_params != "" and total_params != active_params:
            size = f"{total_params}({active_params})"
        else:
            size = f"{total_params}" if total_params != "" else ""
...
if __name__ == "__main__":
    ...
    try:
        ...
        latex_output = generate_all_academic_tables(
            ...
            sum_results,
            models_csv,
            output_file="table_model_total.tex",
        )
